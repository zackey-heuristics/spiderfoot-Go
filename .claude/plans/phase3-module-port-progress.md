...
**Branch**: `feature/1-go-rewrite` — Batch 21 committed locally.
**Last commit**: Batch 21 — breach/paste/search intel (psbdmp, skymem, grepapp, snov, abstractapi).
**Registered modules**: 138 / 234.
**Next batch**: **Batch 22** — to be selected from remaining Python modules. Follow the plan-implement-test-review-commit cycle established in Batches 11-14.
...
| 19 — Passive DNS & Historical Whois | (local) | dnsdb, whoxy, mnemonic, circllu, fullhunt | 5 modules in `passive_dns.go`. `mnemonic` is the only no-key module (free PassiveDNS v3 `api.mnemonic.no/pdns/v3/`). Keyed modules use vendor-prefixed opts: `dnsdb_api_key` (X-API-Key header), `whoxy_api_key` (query param), `circllu_api_key_login`+`circllu_api_key_password` (Basic auth), `fullhunt_api_key` (X-API-KEY header). DNSDB parses the v2 ndjson begin/record/end frame via a dedicated `parseDNSDBNDJSON` helper (tested separately). Whoxy walks paginated reverse-whois results; commits only when `status==1` and at least one domain is returned. Mnemonic commits only when `responseCode==200` AND `size>0` AND at least one record produced an event. CIRCL.LU parses one-JSON-per-line output and commits only when at least one passive-DNS hit matches the target. FullHunt walks `hosts[].dns.{mx,ns,cname}` + `network_ports` and classifies hosts as `INTERNET_NAME` vs `AFFILIATE_INTERNET_NAME` by subdomain match against the queried root; commits only when `hosts` is non-empty. All requests are HTTPS. `TestPassiveDNSWatchedEvents` asserts each module's claimed input types are actually subscribed (Batch 15-18 hard-learned-lesson #2). |
| 21 — Breach / Paste / Search Intel | (local) | psbdmp, skymem, grepapp, snov, abstractapi | 5 modules in `breach_paste_intel.go`. `psbdmp`, `skymem`, `grepapp` are free no-key (psbdmp → LEAKSITE_URL; skymem scrapes emails via shared `sflib.ExtractEmails` + target-domain filter; grepapp walks paginated JSON API with API-reported total as pagination ceiling + visible RAW_RIR_DATA cap-hit notice). `snov` uses the Snov.io two-step flow: `snov_api_key_client_id`+`snov_api_key_client_secret` exchange for an OAuth access token via `/v1/oauth/access_token`, then page through `/v2/domain-emails-with-info`; commits only when at least one EMAILADDR is emitted. `abstractapi` uses three independent vendor-prefixed keys (`abstractapi_api_key_companyenrichment`/`_phonevalidation`/`_ipgeolocation`) — one per upstream endpoint — and is the first Batch 21+ module to dispatch on event type to a different keyed sub-endpoint. All requests forced to HTTPS (skymem's Python version used plaintext). Generic `api_key`/`token`/`client_id` fallback explicitly rejected by `TestBatch21RejectGenericAPIKey`. |
| 20 — Darkweb / Company / Network Intel | (local) | ahmia, onionsearchengine, bitcoinabuse, opencorporates, onyphe, zetalytics | 6 modules in `darkweb_company_intel.go`. `ahmia` and `onionsearchengine` are free Tor search engines scraping HTML for onion URLs (no API key; DARKNET_MENTION_URL only — the DARKNET_MENTION_CONTENT follow-fetch via Tor is deferred since sflib does not yet expose a SOCKS proxy transport). `bitcoinabuse` uses vendor-prefixed `bitcoinabuse_api_key` with query-param auth; commits only when `address` and `count>0` are both set on the documented response. Adds `MALICIOUS_BITCOIN_ADDRESS` event type. `opencorporates` requires `opencorporates_api_key` (no anonymous-50/day fallback — we never ship credentials silently); parses nested `results.companies[].company` shape and emits PHYSICAL_ADDRESS, previous COMPANY_NAME, and RAW_RIR_DATA officer names. `onyphe` hits four v2 simple endpoints (geoloc/pastries/threatlist/vulnscan) under `Authorization: apikey <k>`, commits only when at least one endpoint returned substantive results; CVEs emit as VULNERABILITY_GENERAL (event.cveInfo severity classification deferred). `zetalytics` dispatches on input type (INTERNET_NAME→/hostname, DOMAIN_NAME→/subdomains+/email_domain, EMAILADDR→/email_address) and only emits INTERNET_NAME for hosts matching the scan target (affiliate subdomain filtering). |
| 18 — IP Geolocation & Enrichment APIs | (local) | ipapico, ipapicom, ipregistry, ipstack, neutrinoapi | 5 modules in `geo_ip_apis.go`. `ipapico` is the only no-key module (free ipapi.co `/json/`). Keyed modules use vendor-prefixed opts: `ipapicom_api_key`, `ipregistry_api_key`, `ipstack_api_key`, and the `neutrinoapi_api_key_user`+`neutrinoapi_api_key_secret` pair. All requests forced to HTTPS even where Python used plaintext HTTP (Batch 15-17 review rule). Commit-only-on-substantive-data applied: ipapico checks `error:false` + non-empty `country`; ipapicom/ipstack check documented `success:false` flag before committing; ipregistry requires non-empty nested `location.country.name`/`region.name` or security flag; neutrinoapi commits only if at least one sub-endpoint emits (ip-info geo, ip-blocklist is-listed, or host-reputation is-listed). Python `PROXY_HOST`/`VPN_HOST`/`TOR_EXIT_NODE` sub-events are folded into `BLACKLISTED_IPADDR`+`MALICIOUS_IPADDR` because the Go event type set does not define those types. Nested `ipregistry` shape (`location.country: {name}`, `location.region: {name}`) is mirrored in Go structs verbatim (not flattened) — see `TestIPRegistryNestedShape` regression. NeutrinoAPI hyphenated JSON field names (`country-code`, `is-listed`) handled via struct tags. |

**Total registered modules: 138** (dns_resolve + stor_db pre-existing, +120 new; vxvault + multiproxy dropped post-review)
