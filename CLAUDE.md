...
- **Phase 3** (IN PROGRESS) — Module porting: 138 / 234 modules ported (Batches 0-21 complete). Progress and conventions tracked in `.claude/plans/phase3-module-port-progress.md`. **Next**: Batch 22. See the "Session handoff" block at the top of that file for a resume prompt.
...
- **Batch 16**: More Free Reputation Feeds — talosintel, alienvaultiprep, greensnow, stevenblack. 4 modules in `threat_feeds2.go`. All reuse `ipFeed`/`hostFeed` generics. Adds an optional `parser` field to `ipFeed` so feeds whose wire format is not a plain IP per line (alienvaultiprep `IP #desc`) can override `parseIPList` without duplicating HandleEvent. `stevenblack` parses hosts-file format, iterates every alias on a line, strips inline comments, and skips `localhost` aliases. **Dropped from original plan**: `vxvault` and `multiproxy` — upstream serves plaintext HTTP only; Codex adversarial review flagged ingesting unauthenticated reputation data as a trust-boundary failure (on-path attacker can inject false positives/negatives). Re-enable only if HTTPS sources or a quarantine/opt-in profile is added.
- **Batch 17**: IP/Domain Intel APIs — pulsedive, binaryedge, fraudguard, ipqualityscore, fullcontact, spyonweb. 6 modules in `ip_intel.go`. All require API keys (vendor-prefixed opts). Exercises Bearer (fullcontact), Basic auth pair (fraudguard), custom header (binaryedge X-Key) and query-param (pulsedive, spyonweb, ipqualityscore) auth schemes. fullcontact uses POST to v3 person/company enrich endpoints; ipqualityscore embeds the api key in the URL path.
- **Batch 18**: IP Geolocation & Enrichment APIs — ipapico, ipapicom, ipregistry, ipstack, neutrinoapi. 5 modules in `geo_ip_apis.go`. `ipapico` is free/no-key; the rest use vendor-prefixed opts (`ipapicom_api_key`, `ipregistry_api_key`, `ipstack_api_key`, and the `neutrinoapi_api_key_user`+`neutrinoapi_api_key_secret` pair). All traffic HTTPS even where the Python source used plaintext. Commit-on-substantive-data enforced: ipapico checks `error:false`+non-empty country; ipapicom/ipstack check documented `success:false` before committing; ipregistry requires a non-empty `location.country.name`/`location.region.name` (nested objects, mirrored in Go structs, not flattened) or a security flag; neutrinoapi commits only if at least one sub-endpoint (ip-info geo / ip-blocklist / host-reputation) emits. Python `PROXY_HOST`/`VPN_HOST`/`TOR_EXIT_NODE` sub-events are folded into `BLACKLISTED_IPADDR`+`MALICIOUS_IPADDR` because the Go event registry does not define those types.
- **Batch 19**: Passive DNS & Historical Whois — dnsdb, whoxy, mnemonic, circllu, fullhunt. 5 modules in `passive_dns.go`. `mnemonic` is the only no-key module (PassiveDNS v3). Keyed modules use vendor-prefixed opts: `dnsdb_api_key` (X-API-Key header), `whoxy_api_key` (query param), `circllu_api_key_login`+`circllu_api_key_password` (Basic auth), `fullhunt_api_key` (X-API-KEY header). DNSDB parses the v2 begin/record/end ndjson frame via a dedicated `parseDNSDBNDJSON` helper (separately tested). Whoxy walks paginated reverse-whois-by-email results and commits only when `status==1` with at least one domain. Mnemonic commits only on `responseCode==200 && size>0` with at least one emitted event. CIRCL.LU parses one-JSON-per-line output and only commits on a matching hit. FullHunt walks `hosts[].dns.{mx,ns,cname}` + `network_ports` and classifies hosts as `INTERNET_NAME` vs `AFFILIATE_INTERNET_NAME` by subdomain match. All requests HTTPS. Regression test `TestPassiveDNSWatchedEvents` asserts each module's advertised input types are actually subscribed.
- **Batch 21**: Breach/Paste/Search Intel — psbdmp, skymem, grepapp, snov, abstractapi. 5 modules in `breach_paste_intel.go`. psbdmp/skymem/grepapp are free no-key (psbdmp → LEAKSITE_URL; skymem scrapes emails with target-domain filter; grepapp walks paginated JSON with API-reported total as pagination ceiling + visible cap-hit RAW_RIR_DATA notice). snov uses two-step OAuth (`snov_api_key_client_id`+`_client_secret`) then `/v2/domain-emails-with-info` pagination, committing only when at least one EMAILADDR is produced. abstractapi dispatches on event type to three independent per-endpoint keys (`abstractapi_api_key_companyenrichment`/`_phonevalidation`/`_ipgeolocation`). All requests HTTPS; skymem was HTTP in Python.
- **Batch 20**: Darkweb / Company / Network Intel — ahmia, onionsearchengine, bitcoinabuse, opencorporates, onyphe, zetalytics. 6 modules in `darkweb_company_intel.go`. ahmia + onionsearchengine are free HTML-scrape Tor search engines (DARKNET_MENTION_URL only; follow-fetch via Tor deferred until sflib gains a SOCKS transport). bitcoinabuse/opencorporates/onyphe/zetalytics are keyed with vendor-prefixed opts. Adds the `MALICIOUS_BITCOIN_ADDRESS` event type.
- **Next**: Batch 21 — see `.claude/plans/phase3-module-port-progress.md`.
